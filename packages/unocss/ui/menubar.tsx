...
export const MenubarTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <MenubarPrimitive.Trigger
      class={cn(
        "focus:(bg-accent text-accent-foreground) data-[expanded]:(bg-accent text-accent-foreground) flex cursor-default select-none items-center rounded-sm px-3 py-1 text-sm font-medium outline-none",
        local.class
      )}
      {...rest}
    />
...
export const MenubarSubTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <MenubarPrimitive.SubTrigger
      class={cn(
        "focus:(bg-accent text-accent-foreground) data-[expanded]:(bg-accent text-accent-foreground) flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none",
        local.inset && "pl-8",
        local.class
      )}
      {...rest}
  ...
  return (
    <MenubarPrimitive.Portal>
      <MenubarPrimitive.SubContent
        class={cn(
          "bg-popover text-popover-foreground data-[expanded]:(animate-in fade-in-0 zoom-in-95) data-[closed]:(animate-out fade-out-0 zoom-out-95) z-50 min-w-[8rem] origin-[--kb-menu-content-transform-origin] overflow-hidden rounded-md border p-1 shadow-lg outline-none",
          local.class
        )}
        {...rest}
      >
  ...
  return (
    <MenubarPrimitive.Portal>
      <MenubarPrimitive.Content
        class={cn(
          "bg-popover text-popover-foreground data-[expanded]:(animate-in fade-in-0 zoom-in-95) data-[closed]:(animate-out fade-out-0 zoom-out-95) z-50 min-w-[12rem] origin-[--kb-menu-content-transform-origin] overflow-hidden rounded-md border p-1 shadow-md outline-none",
          local.class
        )}
        {...rest}
      >
...
export const MenubarCheckboxItem = <T extends ValidComponent = "div">(
  ...
  return (
    <MenubarPrimitive.CheckboxItem
      class={cn(
        "focus:(bg-accent text-accent-foreground) data-[disabled]:(pointer-events-none opacity-50) relative flex cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none",
        local.class
      )}
      {...rest}
    >
