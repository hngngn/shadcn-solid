...
export const CheckboxControl = <T extends ValidComponent = "div">(
  ...
  return (
    <>
      <CheckboxPrimitive.Input class="[&:focus-visible+div]:(outline-none ring-1.5 ring-ring ring-offset-background) ring-offset-2" />
      <CheckboxPrimitive.Control
        class={cn(
          "border-primary focus-visible:(outline-none ring-1.5 ring-ring) data-[disabled]:(cursor-not-allowed opacity-50) data-[checked]:(bg-primary text-primary-foreground) h-4 w-4 shrink-0 rounded-sm border shadow transition-shadow",
          local.class
        )}
        {...rest}
      >
