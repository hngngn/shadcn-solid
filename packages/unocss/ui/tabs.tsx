...
export const TabsList = <T extends ValidComponent = "div">(
  ...
  return (
    <TabsPrimitive.List
      class={cn(
        "bg-muted text-muted-foreground data-[orientation=vertical]:(flex-col items-stretch) relative flex w-full rounded-lg p-1 data-[orientation=horizontal]:items-center",
        local.class
      )}
      {...rest}
    />
...
export const TabsContent = <T extends ValidComponent = "div">(
  ...
  return (
    <TabsPrimitive.Content
      class={cn(
        "focus-visible:(ring-offset-background ring-1.5 ring-ring ring-offset-2) outline-none transition-shadow duration-200 data-[orientation=horizontal]:mt-2 data-[orientation=vertical]:ml-2",
        local.class
      )}
      {...rest}
    />
...
export const TabsTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <TabsPrimitive.Trigger
      class={cn(
        "disabled:(pointer-events-none opacity-50) data-[selected]:text-foreground peer relative z-10 inline-flex h-7 w-full items-center justify-center whitespace-nowrap rounded-md bg-transparent px-3 py-1 text-sm font-medium outline-none transition-colors",
        local.class
      )}
      {...rest}
    />
