	...
	return (
		<DrawerPrimitive.Portal>
			...
			<DrawerPrimitive.Content
				class={cn(
					"fixed z-50 flex h-auto border-border bg-background data-[transitioning]:transition-transform data-[transitioning]:duration-500 data-[transitioning]:[transition-timing-function:cubic-bezier(0.32,0.72,0,1)] md:select-none after:absolute after:h-1/2 after:bg-inherit",
					ctx.side() === "right" && [
						"inset-y-0 right-0 rounded-l-lg border-l pl-4",
						"after:inset-y-0 after:left-[calc(100%-1px)]",
					],
					ctx.side() === "bottom" && [
						"inset-x-0 bottom-0 rounded-t-lg border-t pt-4 flex-col",
						"after:inset-x-0 after:top-[calc(100%-1px)]",
					],
					ctx.side() === "left" && [
						"inset-y-0 left-0 rounded-r-lg border-r pr-4",
						"after:inset-y-0 after:right-[calc(100%-1px)] flex-row-reverse",
					],
					ctx.side() === "top" && [
						"inset-x-0 top-0 rounded-b-lg border-b pb-4 flex-col-reverse",
						"after:inset-x-0 after:bottom-[calc(100%-1px)]",
					],
					local.class,
				)}
				{...rest}
