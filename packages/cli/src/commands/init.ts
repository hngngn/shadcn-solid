...
import { object, string, parse as valibotParse } from "valibot";
import {
	DEFAULT_COMPONENTS,
	DEFAULT_CSS,
...
export const initCommand = new Command()
	.name("init")
	.description("initialize and install dependencies")
	.option("-c, --cwd <path>", "the working directory", process.cwd())
	.action(async (opts) => {
		try {
			const parsed = valibotParse(schema, opts);
			...
			const group = await p.group(
				{
					...
					component: () => {
						...
						if (
							...
							p.log.warning(
								cl.yellow(
									`Please make sure to use the same alias defined in your ${cl.bold(
										"tsconfig.json",
									)}`,
								),
							);
						...
						return p.text({
							...
							defaultValue:
								defaultConfig?.alias.component ?? DEFAULT_COMPONENTS,
						});
					},
					cn: () =>
						p.text({
			...
			for (const [, resolvePath] of Object.entries(
				promptConfig.resolvedPaths,
			)) {
			...
			await writeFile(
				...
				template(
					...
					prefix: promptConfig.uno
						? promptConfig.uno.prefix
						: promptConfig.tailwind?.prefix,
				}),
				"utf8",
			);
			...
			const baseColor = await getRegistryColor(
				promptConfig.uno
					? promptConfig.uno.color
					: promptConfig.tailwind?.color,
			...
			if (baseColorConfig !== undefined) {
				if (baseColor) {
					await writeFile(
						promptConfig.resolvedPaths.css,
						baseColorConfig,
						"utf8",
					);
				}
			...
			spinner.start(
				`Installing ${cl.cyan(PROJECT_DEPENDENCIES.map((v) => v).join(", "))}`,
			);
