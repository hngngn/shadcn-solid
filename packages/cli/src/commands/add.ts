...
export const addCommand = new Command()
	...
	.action(async (components, opts) => {
		try {
			...
			let selectComponents = parsed.all
				? registryComponent.map((v) => v.name)
				: parsed.components;
			...
			for (const item of payload) {
				...
				const existingComponent = item.files.filter((v) =>
					existsSync(resolve(targetDir, v.name)),
				);
				...
				if (existingComponent.length && !parsed.overwrite) {
					if (selectComponents?.includes(item.name)) {
						p.log.warning(
							cl.yellow(
								`Component ${item.name} already exists. Use ${cl.bold("-o")} to overwrite`,
							),
						);
						process.exit(1);
					}
