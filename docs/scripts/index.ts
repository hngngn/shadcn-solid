...
import { Type, registrySchema } from "./utils/schema";
...
for (const framework of frameworks) {
	...
	for (const item of result.output) {
		if (item.type === Type["components:example"]) {
			index += `
    "${item.name}": {
      name: "${item.name}",
      type: "${item.type}", 
...
for (const framework of frameworks) {
	...
	for (const item of result.output) {
		if (item.type !== Type["components:ui"]) {
			continue;
		}
...
// ----------------------------------------------------------------------------
const names = result.output.filter(
	(item) => item.type === Type["components:ui"],
);
