...
const makeData = (len: number) =>
  Array.from({ length: len }, (_, i) => i).map(
    (): Task => ({
      ...
      status:
        faker.helpers.shuffle<Task["status"]>(["todo", "in-progress", "done", "cancelled"])[0] ??
        "todo",
      label:
...
const DataTableDemo = () => {
  ...
  onMount(() => {
    setSearchParams({
      ...
      per_page: searchParams.per_page !== undefined ? searchParams.per_page : 10
    });
  });
    ...
    on(
      ...
      ([p, s]) => {
        setSearchParams({
          page: p.pageIndex + 1,
          per_page: p.pageSize,
          sort: s.map(v => (v.id ? `${v.id}.${v.desc ? "desc" : "asc"}` : null)).join(",")
        });
      },
      {
        defer: true
